...
_REWRITE_SYSTEM = (
    _KOREAN_ONLY
    + "너는 학부모 민원을 공적인 표현으로 다듬는다. "
    "학부모가 제기한 문제와 요구를, 감정·공격·과장 표현을 빼고 중립적인 한두 문장으로 요약하라. "
    "교사의 답변·사과·약속·조치를 쓰지 말고, '학부모가 무엇을 문제삼고 무엇을 원하는지'만 적는다. "
    "학생은 민원에 나온 이름을 그대로 쓰고(없으면 '자녀'라고만), '씨'나 '○○' 같은 빈 자리표시자를 절대 쓰지 마라. "
    "요약문만 출력하라."
)

_DRAFT_SYSTEM = (
    _KOREAN_ONLY
    + "너는 교사가 학부모에게 보낼 답변 초안을 쓴다. 학부모를 향한 정중한 글이다.\n"
    "1) 학부모의 감정과 자녀에게 먼저 공감하며 시작한다.\n"
    "2) 아직 사실 확인 전이므로 교사의 잘못을 인정하거나 '앞으로 더 신중히 하겠다'처럼 "
    "잘못을 전제한 약속을 하지 마라. '무슨 일이 있었는지 확인하겠다'는 태도에서 멈춘다.\n"
    "3) 구체적인 후속(면담·확인·연락)을 제안한다.\n"
    "4) 학생은 민원에 나온 이름을 그대로 쓰고(없으면 '자녀'라고만 칭한다), "
    "'씨'·'○○'·'[ ]' 같은 자리표시자나 모르는 정보를 절대 쓰지 않는다. 호칭은 '학부모님'.\n"
